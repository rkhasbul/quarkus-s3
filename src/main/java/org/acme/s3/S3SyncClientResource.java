...
public class S3SyncClientResource extends CommonResource {
  ...
  public Response uploadFile(@MultipartForm FormData formData) throws Exception {
    ...
    PutObjectResponse putResponse =
        s3.putObject(buildPutRequest(formData), RequestBody.fromFile(uploadToTemp(formData.data)));
    if (putResponse != null) {
  ...
  public Response downloadFile(@PathParam("objectKey") String objectKey) {
    ...
    GetObjectResponse object =
        s3.getObject(buildGetRequest(objectKey), ResponseTransformer.toOutputStream(baos));
  ...
  public List<FileObject> listFiles() {
    ...
    return s3.listObjects(listRequest).contents().stream()
        .sorted(Comparator.comparing(S3Object::lastModified).reversed()).map(FileObject::from)
